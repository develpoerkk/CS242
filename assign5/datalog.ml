...
module DatabaseImpl : Database = struct
  ...
  let rec saturate (rules : Rule.t list) (facts : Relation.Set.t) : Relation.Set.t =
    (* here, we need to define the first layer recursion *)
    let rec loop_rules (_rules: Rule.t list) (_facts: Relation.Set.t): Relation.Set.t = (
      match _rules with 
      | [] -> _facts
      | _rule :: _rules' -> (
          (* here we need to define the second layer recursion *)
          let rec loop_relations (relations: Relation.t list) (__facts: Relation.Set.t): Relation.t list list = (
            match relations with       (* _rule.body should be Relation.t list *)
            | [] -> []
            | relation :: relations' -> (applicable_facts __facts relation) :: (loop_relations relations' __facts)
          ) in
          let app_facts = loop_relations (_rule.body) (_facts) in
          let candidates = cross_product (app_facts) in
          let rec loop_candidates (_candidates: Relation.t list list): (Relation.t list) = (
            match _candidates with
            | [] -> []
            | _candidate :: _candidates' -> (
                try
                  let rec loop_zipped (_zipped: (Relation.t * Relation.t) list): env = (
                    match _zipped with
                    | [] -> empty_env
                    | (candidate_relation, rule_relation) :: _zipped' -> unify (loop_zipped _zipped') candidate_relation rule_relation
                  ) in
                  let zipped = (List.zip_exn _candidate _rule.body) in
                  let _env = loop_zipped(zipped) in
                  let new_fact = substitute _rule.head _env in
                  new_fact :: loop_candidates _candidates'
                with
                | CannotUnify -> loop_candidates _candidates'
              )
          ) in
          let new_facts_for_rule = loop_candidates(candidates) in
          Relation.Set.union (Relation.Set.of_list new_facts_for_rule) (loop_rules _rules' facts)
        )
    ) in
    let new_facts = loop_rules (rules) (facts) in
    if (Relation.Set.equal new_facts facts) then 
      facts 
    else 
      saturate rules new_facts
  ...
  let rec query (facts : t) (q : Relation.t) : Relation.t list =
    (* t should be a Relation.t list *)
    match facts with
    | [] -> []
    | fact :: facts' -> (
        try 
          let __env = (unify empty_env fact q) in
          fact :: (query facts' q) 
        with 
        | CannotUnify -> query facts' q
      )
