...
let rec translate_term (t : Lang.Term.t) : IR.Term.t =
  ...
  | Lang.Term.Let (p, arg, body) ->
    ...
    | Lang.Pattern.Var (x, tau) ->
      let t1' = translate_term arg in
      let t2' = translate_term body in
      IR.Term.App (IR.Term.Lam (x, (translate_type tau), t2'), t1')

    | Lang.Pattern.Alias (pp, x, tau) ->
      let t1' = translate_term arg in
      let t' = Lang.Term.Let(pp, arg, body) in
      IR.Term.App (IR.Term.Lam (x, translate_type tau, translate_term t'), t1')

    | Lang.Pattern.Tuple (p1, p2) ->
      let l = Lang.Term.Project (arg, Left) in
      let r = Lang.Term.Project (arg, Right) in 
      let part2 = Lang.Term.Let (p2, r, body) in
      let part1 = Lang.Term.Let (p1, l, part2) in
      translate_term part1

    | Lang.Pattern.TUnpack (cap_x, x) ->
      let t1' = translate_term arg in
      let t2' = translate_term body in
      IR.Term.TUnpack (cap_x, x, t1', t2') 
    )
